	...
	return (
		<Center h='100%' w='100%' p='xl' bg='light-dark(var(--mantine-color-gray-0), var(--mantine-color-dark-6))'>
			<Stack align='center' gap='md'>
				...
				<Text c='light-dark(var(--mantine-color-gray-6), var(--mantine-color-gray-0))'>{message || translate('messages.no.data')}</Text>
			</Stack>
		</Center>
	);
};
...
const NotFound: React.FC = () => {
	const { t: translate } = useTranslation('vending_machine');
	return (
		<EmptyContent message={translate('messages.not.found')} />
	);
};
...
const NoData: React.FC = () => {
	const { t: translate } = useTranslation('vending_machine');
	return (
		<EmptyContent message={translate('messages.no.data')} />
	);
};
