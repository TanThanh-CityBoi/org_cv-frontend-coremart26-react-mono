...
export function calculateCenter(kiosks: Kiosk[]): [number, number] {
	...
	const minLng = Math.min(...lngs.map(Number));
	const maxLng = Math.max(...lngs.map(Number));
	const minLat = Math.min(...lats.map(Number));
	const maxLat = Math.max(...lats.map(Number));
...
export function createBoundsFromKiosks(kiosks: Kiosk[]): maplibregl.LngLatBounds | null {
	...
	return new maplibregl.LngLatBounds(
		[Math.min(...lngs.map(Number)), Math.min(...lats.map(Number))],
		[Math.max(...lngs.map(Number)), Math.max(...lats.map(Number))],
	);
}
