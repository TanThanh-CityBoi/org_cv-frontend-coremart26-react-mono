	...
	React.useEffect(() => {
		...
		if (archiveState.status === 'success') {
			...
			const messageKey = archived
				? 'coremart.vendingMachine.events.messages.archive_success'
				: 'coremart.vendingMachine.events.messages.restore_success';
			...
			handleCloseModal();
			onArchiveSuccess();
			dispatch(eventActions.resetArchiveEvent());
			return;
		}
...
export const useEventArchive = ({
	...
	const archiveState = useMicroAppSelector(selectEventArchiveOutcome);
	...
	const handleConfirmArchive = React.useCallback(() => {
		...
		const pendingAction = dispatch(eventActions.setArchivedEvent({ id, etag, isArchived: targetArchived }));
