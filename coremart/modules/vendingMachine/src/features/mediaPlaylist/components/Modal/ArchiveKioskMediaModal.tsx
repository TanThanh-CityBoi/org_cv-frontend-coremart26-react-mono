...
export const ArchiveKioskMediaModal: React.FC<ArchiveKioskMediaModalProps> = ({
	...
	return (
		<ConfirmModal
			title={isArchive
				? translate('kiosk_media.messages.archive_modal_title')
				: translate('kiosk_media.messages.restore_modal_title')}
			opened={opened}
			onClose={onClose}
			onConfirm={onConfirm}
			message={
				<Trans
					i18nKey={isArchive
						? 'kiosk_media.messages.archive_confirm'
						: 'kiosk_media.messages.restore_confirm'}
					values={{ name }}
