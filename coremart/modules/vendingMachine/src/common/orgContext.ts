 ...
 *
 * Backend reads in this module are org-scoped: the search endpoints bind `org_id` from a query
 * param and filter on it unconditionally, so omitting it matches no rows rather than matching
 * all of them. Every call that lists or fetches records must carry it.
 *
